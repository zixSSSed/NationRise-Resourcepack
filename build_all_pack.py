...
# Папка ресурспаков клиента переезжает вместе с лаунчером, поэтому не один жёсткий путь,
# а список: берём первую существующую. Свою можно задать переменной среды NR_RESOURCEPACKS,
# и тогда правка скрипта не нужна вовсе.
CLIENT_DIRS = [
    Path(r"C:\Delta\1.21.11\game\resourcepacks"),
    Path(r"C:\Rockstar\game\resourcepacks"),
    Path.home() / "AppData" / "Roaming" / ".minecraft" / "resourcepacks",
]


def client_dir():
    env = os.environ.get("NR_RESOURCEPACKS")
    if env:
        return Path(env)
    for d in CLIENT_DIRS:
        if d.is_dir():
            return d
    raise SystemExit("не нашёл папку ресурспаков клиента — задайте NR_RESOURCEPACKS или добавьте путь в CLIENT_DIRS")


OUT = client_dir() / "NationRise-ALL.zip"
